...
//		}
		...
		$content = $MCount->getWelcome($today);
		//取数据
	...
	function actionTrend(){
		$MCount = new Count();
		$today=$_GET['t1'] ? $_GET['t1'] : urldecode(arg('today',date("Y-m-d")));
		$domain=$_GET['urls'] ? $_GET['urls'] : '';


		$content = $MCount->getWelcome($today,$domain);
		//取数据
		$this->tpl_today = $content['today'];
		...
		$this->today=$today;
		$this->pretoday = date("Y-m-d",(strtotime($today) - 3600*24));
	...
	function actionContrast() {
		...
		//取今日数据
		$this->tpl_today = $MCount->getContrast($today,$domain);
		//取明日数据
		$this->tpl_yesterday = $MCount->getContrast($yesterday,$domain);
		//dump($this->tpl_today);
		//dump($this->tpl_yesterday);
		$this->today=$today;
	...
	function actionTodayApiWebList() {
		...
		foreach ($allDomain as $key => $value) {
			$todayContent = $MCount->getDomainOnceCount($value['domain'],$today);
			$todayContent['domain'] = '<a href="Trend&urls='.$value['domain'].'">'.$value['domain'].'</a><a href="Contrast&urls='.$value['domain'].'">域名对比</a>';
			$todayContent['group'] = $value['group'];
			...
			if(empty($todayContent['shareend'])){
				$todayContent['shareend']=1;
			}
